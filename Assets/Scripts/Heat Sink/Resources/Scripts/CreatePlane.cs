...
public class CreatePlane : ScriptableWizard
{
    ...
    [MenuItem("GameObject/Create Other/Heat Sink Plane")]
    ...
    {
		string ret;		
		ret = AssetDatabase.CreateFolder("Assets/Heat Sink/Resources","Plane");
		if(AssetDatabase.GUIDToAssetPath(ret) != "Assets/Heat Sink/Resources/Plane")
			AssetDatabase.DeleteAsset(AssetDatabase.GUIDToAssetPath(ret));
		...
		meshRenderer.material = Resources.Load("Materials/Diffuse", typeof(Material)) as Material;
		plane.AddComponent(typeof(ColorMapping));
		...
		string planeAssetName = plane.name + "_W" + width + "L" + length + (orientation == Orientation.Horizontal? "H" : "V") + anchorId + ".asset";
		Mesh m = (Mesh)AssetDatabase.LoadAssetAtPath("Assets/Heat Sink/Resources/Plane/" + planeAssetName,typeof(Mesh));
        ...
        if (m == null)
        {
			...
			AssetDatabase.CreateAsset(m, "Assets/Heat Sink/Resources/Plane/" + planeAssetName);
		...
		plane.AddComponent<HeatTransfer>();
